...
// adding audio for fire crackling sound
const fireCrackling = new Audio('/static/firecrackling.m4a');
let pickr;
...
socket.on('connect', () => {
  socket.on('hex', (val) => { document.body.style.backgroundColor = val;
    // play audio on color change
    fireCrackling.currentTime = 0;
    fireCrackling.play();
   })
...
light.onclick = () => {
  ...
  fireCrackling.muted = true;
  fireCrackling.play().then(fireCrackling.muted = false)
